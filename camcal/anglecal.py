...
def cal(jpg):
    ...
    anglex, anglez = 0, 0, #+np.pi/4,  np.pi/4
    while True:
        Rdown = cv2.Rodrigues(np.float32([-np.pi/2 + anglex, 0, anglez]))[0]
        print("Rdown")
        print(Rdown)
        udm1, udm2 = cv2.fisheye.initUndistortRectifyMap( camera_matrix, dist_coeffs, Rdown, new_camera_matrix, (1280, 960), cv2.CV_16SC2)
        #udm1, udm2 = cv2.fisheye.initUndistortRectifyMap( camera_matrix, dist_coeffs, Rdown, new_camera_matrix, (1280, 720), cv2.CV_16SC2)
